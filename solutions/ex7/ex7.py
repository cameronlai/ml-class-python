...
def findClosestCentroids(X, centroids):
    ...
    #               range 1..K
    for i in xrange(m):
        diff = np.sum(np.power(X[i, :] - centroids, 2), axis=1)
...
def computeCentroids(X, idx, K):
    ...
    #               centroid i.
    centroid_mat = np.zeros((K,m))
    centroid_num = np.zeros((K,1))
    for i in xrange(m):
        centroid_mat[idx[i][0], i] = 1
        centroid_num[idx[i][0]] += 1
    centroids = centroid_mat.dot(X) / centroid_num
    # ===========================================
    return centroids
