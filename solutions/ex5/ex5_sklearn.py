...
def learningCurve(X, y, Xval, yval, lambda_val):
    ...
    score_train = np.zeros(train_size)
    score_val = np.zeros(train_size)
    ...
    regr = linear_model.Ridge(alpha=lambda_val, max_iter=400)
    cv = np.array([[np.arange(train_size),
                    np.arange(train_size, m)]]) # Fixed generator
    train_sizes, train_scores, val_scores = learning_curve(
        regr, Xin, yin, cv=cv, train_sizes=np.arange(1, train_size+1))
...
def validationCurve(X, y, Xval, yval):
    ...
    train_scores, val_scores = validation_curve(
        regr, Xin, yin, param_name='alpha', param_range=lambda_vec, cv=cv)
    score_train = train_scores
    score_val = val_scores
    # ===========================================
    return lambda_vec, score_train, score_val
...
if __name__ == "__main__":
    ...
    score_train, score_val = learningCurve(X, y, Xval, yval, lambda_val)
    ...
    plt.ylabel('Score')
    plt.plot(np.arange(1, m+1), score_train, label='Train score')
    plt.plot(np.arange(1, m+1), score_val, label='Validation score')
    plt.legend()

    print('# Training Examples\tTrain Score\tCross Validation Score')
    for i in xrange(m):
        print('  \t%d\t\t%f\t%f' % (i+1, score_train[i], score_val[i]))
    ...
    score_train, score_val = learningCurve(X_poly, y, X_poly_val, yval, lambda_val)
    ...
    lambda_vec, score_train, score_val = validationCurve(X_poly, y, X_poly_val, yval)

    plt.close('all')
    plt.figure()
    plt.xlabel('lambda')
    plt.ylabel('Score')
    plt.plot(lambda_vec, score_train, label='Train')
    plt.plot(lambda_vec, score_val, label='Cross Validation')
    plt.legend()

    print('# lambda\tTrain Score\tCross Validation Score')
