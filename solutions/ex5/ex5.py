...
def linearRegCostFunction(X, y, theta, lambda_val):
    ...
    h_val = np.dot(X, theta).reshape(-1, 1)
    J = np.sum( np.power(h_val-y, 2) )
    J += lambda_val * np.sum ( np.power(theta[2:], 2) )
    J = J / (2 * m)
    # ===========================================
    return J
...
def linearRegGradFunction(X, y, theta, lambda_val):
    ...
    #               regression for a particular choice of theta.
    h_val = np.dot(X, theta).reshape(-1, 1)
    grad = np.dot( np.transpose(X), (h_val-y) ) / m 
    grad[1:] = grad[1:] + lambda_val * np.power(theta[1:], 2).reshape(-1, 1) / m
    # ===========================================
    grad = grad.ravel()
    return grad
...
def learningCurve(X, y, Xval, yval, lambda_val):
    ...
    for i in xrange(m):
        train_X = X[0:i+1, :]
        train_y = y[0:i+1]
        theta = trainLinearReg(train_X, train_y, lambda_val)
